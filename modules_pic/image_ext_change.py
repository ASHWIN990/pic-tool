...
def main():
    ...
    if os.name == 'nt':
        parser = argparse.ArgumentParser(description=f"Image Extension Changer, Writeen by : ASHWINI SAHU, GITHUB : https:github.com/ASHWIN990/pic-tool.git")
    ...
    if args.dir == True and args.single == False:
        ...
        if os.path.isdir(args.img):
            if os.name == 'nt':
                dir_image_ext_changer_window(args)
            else:
                dir_image_ext_changer_other(args)
        else:
            print(f"Path {args.img} : is invalid")
...
def dir_image_ext_changer_window(args):

    # Note: For Windows while giving the path to the image or folder give paths as :-
    # Example : F:\\Wallpaper\\Nature\\Forest.jpg      With double \\

    img_dir_path=int(args.img)
    new_ext=str(args.ext)
...
def dir_image_ext_changer_other(args):
