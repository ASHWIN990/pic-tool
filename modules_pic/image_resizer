...
def main():
    ...
    if os.name == 'nt':
        parser = argparse.ArgumentParser(description=f"Image Resizer, Writeen by : ASHWINI SAHU, GITHUB : https:github.com/ASHWIN990/pic-tool.git")
        
    else:
        parser = argparse.ArgumentParser(description=f"Image Resizer, Writeen by : {blue_col}ASHWINI SAHU{reset_col}, GITHUB : https:github.com/ASHWIN990/pic-tool.git")
    ...
    if args.dir == True and args.single == False:
        ...
        if os.path.isdir(args.img):
            if os.name == 'nt':
                dir_image_resize_window(args)
            else:
                dir_image_resize_other(args)
        else:
            print(f"Path {args.img} : is invalid")

def single_img_resize(args):
    ...
    resized_image = image.resize((img_wid, img_hei)) ## Width, Height
    resized_image.save(f'resized_{img_basename}')
    print("Done ✓")
    exit(0)

def dir_image_resize_window(args):
    ...
    for files in list_of_files:
        if os.path.isfile(f"{img_dir_path}\\\\{files}"):
            try:
                ...
                resized_image = image.resize((img_wid, img_hei))
                resized_image.save(f'resized_{files}')
    print("\nDone ✓")
    exit(0)

def dir_image_resize_other(args):
