...
def main():
    ...
    if os.name == 'nt':
        parser = argparse.ArgumentParser(description=f"Image Thumbnailer, Writeen by : ASHWINI SAHU, GITHUB : https:github.com/ASHWIN990/pic-tool.git")
    else:
    ...
    if args.dir == True and args.single == False:
        ...
        if os.path.isdir(args.img):
            if os.name == 'nt':
                dir_image_thumbnailer_window(args)
            else:
                dir_image_thumbnailer_other(args)
        else:
            print(f"Path {args.img} : is invalid")
...
def dir_image_thumbnailer_window(args):
...
def dir_image_thumbnailer_other(args):
